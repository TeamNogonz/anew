...
def build_result(document, preferences, now):
    ...
    for raw in data.get("summary_items", []):
        ...
        if preferences.keywords and not any(
            word.casefold() in serialized for word in preferences.keywords
        ):
            continue
        references = [
            url
            for url in raw.get("reference_url", [])
            if isinstance(url, str)
            and urlparse(url).scheme in ("https", "http")
        ...
        for key in ("first_perspective", "second_perspective"):
            ...
            item[key] = {
                "title": str(perspective.get("title", "관점"))[:100],
                "icon": str(perspective.get("icon", ""))[:10],
                "perspectives": [
                    str(p)[:1000] for p in perspective.get("perspectives", [])[:5]
                ],
            }
        items.append(item)
    ...
    event_id = hashlib.sha256(
        json.dumps(
            [
                {"title": item["title"], "urls": sorted(item["reference_url"])}
                for item in items
            ],
            ensure_ascii=False,
            sort_keys=True,
    ...
    return {
        ...
        "title": f"Anew · 오늘의 뉴스 {len(items)}개",
        "body": " · ".join(item["title"] for item in items)[:200],
        "expires_at": expires,
        "content": {"summary_items": items, "created_at": created},
    }
