...
async def lifespan(app):
    ...
    if (
        ...
        and settings.nudger_service_token.startswith(("change-me", "local-demo"))
    ):
        raise RuntimeError("Example service credentials are prohibited in production")
    if settings.environment == "production" and settings.schedule_enabled:
        raise RuntimeError("Run python -m collector separately from production API")
    scheduler = None
    thread = None
